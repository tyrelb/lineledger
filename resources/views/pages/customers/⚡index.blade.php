...
new #[Title('Customers')] class extends Component {
    ...
    public function openStatement(int $id): void
    {
        $customer = Contact::where('is_customer', true)->findOrFail($id);
        abort_unless($customer->company_id === $this->company->id, 403);
        ...
        $this->statementAsOf = $today->toDateString();
        $this->statementEnd = $today->toDateString();
        $this->statementStart = $today->startOfYear()->toDateString();
                    ...
                    <tr data-test="customer-row" class="@if(! $customer->is_active) opacity-50 @endif">
                        ...
                        <td class="px-4 py-2 text-right font-mono">
                            <flux:tooltip :content="__('Generate a statement')">
                                <button type="button" wire:click="openStatement({{ $customer->id }})" class="hover:underline" data-test="customer-open-balance">{{ number_format($customer->ar_balance_cents / 100, 2) }}</button>
                            </flux:tooltip>
                        </td>
                        <td class="px-4 py-2 text-right">
                            <flux:dropdown align="end">
                                ...
                                <flux:menu>
                                    ...
                                    <flux:menu.item icon="document-text" wire:click="openStatement({{ $customer->id }})" data-test="customer-statement-button">
                                        {{ __('Statement…') }}
                                    </flux:menu.item>
        ...
        <div class="space-y-6">
            ...
            @if ($statementCustomerId)
                ...
                <div class="flex gap-2">
                    ...
                    <flux:button :href="route('customers.statement.download', $statementParams)" icon="arrow-down-tray" data-test="statement-download">
                        {{ __('Download') }}
                    </flux:button>
                </div>
            @endif
